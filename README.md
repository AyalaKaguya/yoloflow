...
工作区是用户进行项目开发和管理的主要区域，包含数据集工作室、模型管理器、训练计划管理与导出这三个个核心模块，其中训练计划管理模块是核心中的核心，在该模块下还应包含每个训练计划的计划看板、训练任务(数据集选择、模型选择、超参数设定、中断管理)、训练历史、导出等功能。

这部分三个板块的名称还没有想好，先解决项目创建路径的歧义性、为界面添加边框和下载模型失败后的提示信息。
