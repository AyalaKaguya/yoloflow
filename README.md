...
- 模型配置项中，需要结合上一步确定的项目类型由用户选择需要使用的模型，这个模型由yoloflow内部的配置文件列出，形式为"yolo11n-obb.pt":"YOLO 11 选择检测框任务 小尺寸模型"，用户根据后面的选项选择前面的模型文件名称。

1. 创建新项目

项目类型同Ultralytics，包含图像分类、目标检测、语义分割、实例分割、关键点跟踪以及旋转检测框这些任务类别，当选择项目模板指定项目路径后，自动向项目管理器添加关于这个项目的记录，并初始化项目。创建项目向导窗口符合Tab布局，tab栏在左侧，包含项目信息（名称、描述、路径）、项目类型、数据集配置、模型配置选项，初始在项目信息页面，用户可以输入项目名称、描述和路径，路径可以通过浏览按钮选择，点击创建后自动创建项目文件夹，并在项目管理器中添加该项目记录，项目类型需要展示图片、标题以及描述。以下一步和Tab选项组织各个页面的转换，当进行到最后一步时下一步改为创建按钮，点击后创建项目并打开项目。
